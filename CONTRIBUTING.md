...
```bash
git clone https://github.com/narduk-enterprises/boat-search.git my-project
cd my-project
pnpm install
...
| --------------- | ---------- | -------------- |
| `boat-search`   | `dev`      | Shared secrets |
| `boat-search`   | `dev_auth` | `example-auth` |

**First-time setup for example-auth:**

```bash
cd apps/example-auth
doppler setup --project boat-search --config dev_auth --no-interactive
```

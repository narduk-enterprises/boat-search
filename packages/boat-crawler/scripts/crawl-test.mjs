...
async function main() {
  ...
  if (!token || !accountId) {
    ...
    console.error('Set CLOUDFLARE_CRAWL_TOKEN (or CLOUDFLARE_API_TOKEN_WORKERS) and CLOUDFLARE_ACCOUNT_ID');
    console.error('Or run: npm run crawl:test:doppler');
    process.exit(1);
  }

  const testUrl = process.argv[2] || 'https://www.boats.com/boats-for-sale/?length=40-60&type=fishing';
  ...
  try {
    ...
    if (e.message.includes('Browser Rendering') || e.message.includes('10000') || e.message.includes('permission')) {
      console.error('Token may not have "Browser Rendering - Edit". Create a custom API token at https://dash.cloudflare.com/profile/api-tokens with that permission and set CLOUDFLARE_CRAWL_TOKEN.');
    }
    throw e;
  }
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    console.log(`Poll ${i + 1}: status=${status} total=${result.total ?? '?'} finished=${result.finished ?? '?'} browserSecondsUsed=${result.browserSecondsUsed ?? '?'}`);
    if (status !== 'running') {
      if (status === 'completed') {

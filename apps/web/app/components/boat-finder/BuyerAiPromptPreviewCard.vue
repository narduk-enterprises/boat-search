...
<template>
  <!-- Trigger button -->
  <UButton
    label="View AI payload"
    icon="i-lucide-bot-message-square"
    color="neutral"
    variant="soft"
    size="sm"
    data-testid="ai-payload-trigger"
    @click="isOpen = true"
  />
  ...
  <UModal
    v-model:open="isOpen"
    :ui="{
      content: 'max-w-2xl max-h-[85vh] overflow-y-auto',
    }"
  >
    <template #header>
      <div class="flex items-center gap-2" data-testid="ai-payload-modal-header">
        <UIcon name="i-lucide-bot-message-square" class="size-5 text-primary" />
        <span class="text-base font-semibold text-default">Shortlist AI — buyer payload</span>
      </div>
    </template>

    <template #body>
      <div class="space-y-6 p-1" data-testid="ai-payload-modal-body">
        <UAlert
          color="neutral"
          variant="subtle"
          icon="i-lucide-info"

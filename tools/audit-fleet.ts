...
const TEMPLATE_DIR = join(process.env.HOME || '~', 'new-code/boat-search')
...
jobs:
  quality:
    uses: narduk-enterprises/boat-search/.github/workflows/reusable-quality.yml@main
    secrets:
...
function main() {
  ...
  try {
    const gscRaw = execSync(
      'doppler secrets get GSC_SERVICE_ACCOUNT_JSON --project boat-search --config prd --plain',
      { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] },
    ).trim()

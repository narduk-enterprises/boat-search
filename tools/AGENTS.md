...
| `refresh-representative-boat-seed.ts` | Pull a representative subset of enriched production `boats` rows into `apps/web/drizzle/seed-inventory-sample.sql` for local dev seeding                                                                                   | `pnpm run seed:boats:refresh-sample`                                                                                                         |
| `validate-boat-inventory.ts` | D1 field/image aggregates + R2 checks; optional `--urls-missing-description` / `--urls-missing-year` / `--urls-empty-images` prints YachtWorld listing URLs (one per line) for extension backfill | `pnpm run boats:inventory:validate` (local D1); `boats:inventory:validate:production` wraps `doppler run --config prd` for remote D1 + token |
